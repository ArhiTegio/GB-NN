...
from lasagne.layers import ConcatLayer, NonlinearityLayer, DropoutLayer, get_all_layers
...
def build_lstm_network(rnnmodel):
    ...
    net['rnn'] = rnnmodel(net['input'], hidden_units, forgetgate=lasagne.layers.Gate(b=lasagne.init.Constant(1.)),
                          peepholes=False, only_return_final=True, grad_clipping=args.gradclipvalue)
...
def build_rnn_network(rnnmodel):
    net = {}
    net['input'] = InputLayer((batch_size, seq_len, feature_size))
    net['rnn'] = rnnmodel(net['input'], hidden_units, nonlinearity=act, W_in_to_hid=Normal(args.ini),
                          W_hid_to_hid=lambda shape: np.identity(hidden_units, dtype=np.float32),
                          only_return_final=True, grad_clipping=args.gradclipvalue)
    net['out'] = DenseLayer(net['rnn'], outputclass, nonlinearity=softmax)
    return net
...
def build_res_rnn_network(rnnmodel):
    ...
    for l in range(1, num_layers + 1):
        ...
        net['rnn%d' % (l - 1)] = DenseLayer(net['rnn%d' % (l - 1)], hidden_units, W=ini_W,
                                            b=Uniform(range=(0, args.ini_b)),
        ...
        if not args.use_bn_afterrnn:
            net['rnn%d' % l] = lasagne.layers..BatchNormLayer(net['rnn%d' % l], axes=(0, 1), beta=Uniform(range=(0, args.ini_b)))

        net['rnn%d' % l] = rnnmodel(net['rnn%d' % l], hidden_units, W_hid_to_hid=Uniform(range=(hidini, U_bound)),
                                    nonlinearity=act, only_return_final=False, grad_clipping=args.gradclipvalue)
        if args.use_bn_afterrnn:
            net['rnn%d' % l] = lasagne.layers.BatchNormLayer(net['rnn%d' % l], axes=(0, 1))
        if l == num_layers:
...
if args.use_weightdecay_nohiddenW:
    ...
    for para in params:
        if para.name != 'hidden_to_hidden.W':
            loss += args.decayrate * lasagne.regularization.apply_penalty(para,
                                                                          lasagne.regularization.l2)  # *T.clip(T.abs_(para)-1,0,100))
...
updates = opti(grads, params,
               learning_rate=learning_ratetrain)  # nesterov_momentum( loss, params, learning_rate=learning_ratetrain)#
...
bn_test_acc = T.mean(lasagne.objectives.categorical_accuracy(bn_test_prediction, y_sym, top_k=1),
